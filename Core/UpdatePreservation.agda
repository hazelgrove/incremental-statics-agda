...
module Core.UpdatePreservation where
  ...
  PreservationStepAna (WTFun (NTArrowC {d} {n} marrow) (■~N-pair {t} (~N-pair consist)) consist-ana consist-asc consist-body consist-syn consist-all consist-m-all ana) (StepAnnFun {e-body' = _ ⇒ (syn-body' , _)} vars-syn) 
    = WTFun (NTArrowC marrow) (■~N-pair (~N-pair consist)) (▷Pair ▶New)  ▶New ▶New other-random-helper (~N-pair (proj₂ (~D-dec _ _))) ▶New-max-r (preservation-vars-ana?-step ana vars-syn)
