...
module Core.ActionPreservation where
  ...
  SynSubsume (WFAsc x x₁ x₂) = SubsumableAsc
  ...
  ... | _ , consist1 | _ , consist2 = WFFun (NTArrowC consist) (■~N-pair (~N-pair consist1)) (▷Pair ▶★) ▶★ ▶★ NUnless-dirty-▷ (~N-pair ~DVoidL) ▶★ (dirty-ana ana)
  ...
  PreservationStep (WFSubsume subsumable consist-t consist-m (WFAsc consist-syn consist-ana ana)) (ALC ActUnwrapAsc) = dirty-ana ana
  ...
  PreservationStep (WFFun marrow consist consist-ana consist-asc consist-body consist-syn consist-all consist-m-all ana) (ALC (ActUnwrapFun in-ctx var-update)) = dirty-ana (preservation-vars-unwrap in-ctx ana var-update)
  ...
  PreservationStep (WFFun (NTArrowC marrow) (■~N-pair (~N-pair consist)) consist-ana consist-asc consist-body consist-syn (~N-pair consist-all) consist-m-all ana) (ALC (ActDeleteBinder in-ctx var-update)) 
    = WFFun (NTArrowC marrow) (■~N-pair (~N-pair consist)) (▷Pair ▶★) ▶★ ▶★-max-r NUnless-dirty-▷ (~N-pair (proj₂ (~D-dec _ _))) ▶★-max-r (dirty-syn-inner (preservation-vars-unwrap in-ctx ana var-update))
  PreservationStep (WFFun (NTArrowC marrow) (■~N-pair (~N-pair consist)) consist-ana consist-asc consist-body consist-syn (~N-pair consist-all) consist-m-all ana) (ALC (ActInsertBinder var-update)) 
    = WFFun (NTArrowC marrow) (■~N-pair (~N-pair consist)) (▷Pair ▶★) ▶★ ▶★-max-r NUnless-dirty-▷ (~N-pair (proj₂ (~D-dec _ _))) ▶★-max-r (dirty-syn-inner (preservation-vars-ana? ana var-update))
  ...
  mutual 
    ...
    PreservationAna (WFFun {t-asc = t-asc} marrow consist consist-ana consist-asc consist-body consist-syn consist-all consist-m-all ana) (ALowUp (AUpMid (AMidFun {e' = (e' ⇒ _) [ _ ]⇐ _} step))) 
      = WFFun marrow consist (beyond-▷-contra (beyond-AL↦ step) consist-ana) consist-asc consist-body  (preservation-lambda-lemma {t = t-asc} (beyond-AL↦-inner step) consist-syn) consist-all consist-m-all (PreservationAna ana step)
