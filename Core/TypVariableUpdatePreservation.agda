...
module Core.TypVariableUpdatePreservation where
  ...
  mutual 
    ...
    --   = WFFun (preservation-vars-unwrap-t typ inctx tvu inv) x₁ (■~N-pair (~N-pair (proj₂ (~D-dec _ _)))) x₃ x₄ ▶★ (consist-unless-new x₆) x₇ x₈ (ctx-tequiv-ana ana (TInCtxSkip? inctx) etvu (CVCons? inv =▷★))
  ...
  mutual 
    ...
    preservation-vars-unwrap-exp-t-ana (WFFun typ x₁ x₂ x₃ x₄ x₅ x₆ x₇ x₈ ana) inctx (ETVUFun {e-body' = e' ⇒ syn'} tvu etvu) inv 
      = WFFun (preservation-vars-unwrap-t typ inctx tvu inv) x₁ (■~N-pair (~N-pair (proj₂ (~D-dec _ _)))) x₃ x₄ ▶★ (consist-unless-new x₆) x₇ x₈ (preservation-vars-unwrap-exp-t-ana ana (TInCtxSkip? inctx) etvu (CVCons? inv =▷★))
