...
data VarsSynthesize : ℕ -> NewType -> ExpUp -> ExpUp -> Set where 
  ...
  VSFun : ∀ {x t syn asc ana m1 m2 e e'} ->
    ...
    VarsSynthesize x t ((EFun asc m1 (e [ m2 ]⇐ ana)) ⇒ syn) ((EFun asc m1 (e' [ m2 ]⇐ ana)) ⇒ syn)
  ...
  VSAsc : ∀ {x t syn asc e e' ana m} ->
    ...
    VarsSynthesize x t ((EAsc asc (a [ m ]⇐ ana)) ⇒ syn) ((EAsc asc (e' [ m ]⇐ ana)) ⇒ syn)

data _L↦_ : ExpLow -> ExpLow -> Set where 
  -- NewSyn Steps
  StepNewSynConsist : ∀ {t1 t2 m m' n e} ->
    IsNew n ->
    t1 ~M t2 , m' ->
    (e ⇒ (■ (t2 , n))) [ m ]⇐ (⇓ (t1 , Old)) L↦
    (e ⇒ (■ (t2 , Old))) [ m' ]⇐ (⇓ (t1 , Old)) 
  ...
  StepAnaFun : ∀ {t t1 t2 tasc n n1 n2 nasc m1 m2 m3 syn ana e} ->
    ...
    ELow (⇓ (t , Old)) ✔ ((EFun (tasc , nasc) ✔ (ELow (⇓ (t2 , n2)) m3 e)) ⇒ □)
  ...
  StepAnaFunFail2 : ∀ {t asc n m1 m2 m3 ana e} ->
    ...
    ELow (⇓ (t , Old)) ✔ ((EFun asc ✔ (ELow ̸⇓ ✔ e)) ⇒ □)
  StepNoAnaFun : ∀ {asc m1 m2 ana e} ->
    ELow ̸⇓ ✔ ((EFun asc m1 (ELow (⇓ ana) m2 e)) ⇒ □) L↦
    ELow ̸⇓ ✔ ((EFun asc m1 (ELow ̸⇓ ✔ e)) ⇒ □)
...
data _U↦_ : ExpUp -> ExpUp -> Set where 
  ...
  StepNewAnnFun1 : ∀ {t1 n1 m t2 n2 e e'} ->
    ...
    (EFun (t1 , n1) ✔ (ELow ̸⇓ m (e ⇒ (■ (t2 , n2))))) ⇒ □ U↦
    (EFun (t1 , Old) ✔ (ELow ̸⇓ m e')) ⇒ (■ (TArrow t1 t2 , New))
  StepNewAnnFun2 :  ∀ {n oldt1 oldt2 oldn1 oldn2 t1 n1 m t2 n2 e e'} ->
    IsNew n2 ->
    n ▸NArrow oldn1 , oldn2 ->
    VarsSynthesize 0 (t1 , n1) (e ⇒ (■ (t2 , n2))) e' ->
    (EFun (t1 , n1) ✔ (ELow ̸⇓ m (⇒ (■ (t2 , n2)) e))) ⇒ (■ (TArrow oldt1 oldt2 , n)) U↦
    (EFun (t1 , Old) ✔ (ELow ̸⇓ m e')) ⇒ (■ (TArrow t1 oldt2 , NArrow n1 oldn2))
  StepNewSynFun : ∀ {t1 n1 m t2 n2 e syn syn'} ->
    IsNew n2 ->
    MergeSyn (TArrow t1 t2 , NArrow Old n2) syn syn' -> 
    (EFun (t1 , n1) ✔ (ELow ̸⇓ m (e ⇒ (■ (t2 , n2))))) ⇒ syn U↦
    (EFun (t1 , n1) ✔ (ELow ̸⇓ m (e ⇒ (■ (t2 , Old))))) ⇒ (■ syn')
  -- StepNewSynFun2 : ∀ {n oldt1 oldt2 oldn1 oldn2 t1 n1 m t2 n2 e} ->
  --   IsNew n2 ->
  --   n ▸NArrow oldn1 , oldn2 ->
  --   ⇒ (■ (TArrow oldt1 oldt2 , n)) (EFun (t1 , n1) ✔ (ELow ̸⇓ m (⇒ (■ (t2 , n2)) e))) U↦
  --   ⇒ (■ (TArrow oldt1 t2 , NArrow oldn1 n2)) (EFun (t1 , n1) ✔ (ELow ̸⇓ m (⇒ (■ (t2 , Old)) e)))
  StepVoidSynFun : ∀ {t1 n1 m t2 n2 e} ->
    (EFun (t1 , n1) ✔ (ELow ̸⇓ m (e ⇒ (■ (t2 , n2))))) ⇒ □ U↦
    (EFun (t1 , n1) ✔ (ELow ̸⇓ m (e ⇒ (■ (t2 , Old))))) ⇒ (■ (TArrow t1 t2 , New))
  -- Ap Steps
  StepAp : ∀ {t n t1 t2 syn syn' ana ana' e1 e2 m1 m1' mn m2} ->
    ...
    MergeSyn t2 syn syn' -> 
    MergeAna t1 ana ana' -> 
    (EAp (ELow ̸⇓ ✔ (⇒ (■ (t , n)) e1)) m1 (ELow ana m2 e2)) ⇒ syn U↦
    (EAp (ELow ̸⇓ ✔ (⇒ (■ (t , Old)) e1)) m1' (ELow (⇓ ana') m2 e2)) ⇒ (■ syn')
  -- Asc Step
  StepAsc : ∀ {syn syn' t n ana ana' m e} ->
    IsNew n ->
    MergeSyn (t , n) syn syn' -> 
    MergeAna (t , n) ana ana' -> 
    (EAsc (t , n) (ELow ana m e)) ⇒ syn U↦
    (EAsc (t , Old) (ELow (⇓ ana') m e)) ⇒ (■ syn')
